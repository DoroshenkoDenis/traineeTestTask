...
// наследование - классы роботов наследуются от класса AbstractRobot
// инкапсуляция - поля собраны вместе и скрыты от доступа из вне, доступ к приватным полям через методы геттеры-сеттеры
// полиморфизм - потомки могут создавать экземпляры с разными полями
